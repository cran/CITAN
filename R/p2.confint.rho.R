...
#' @export
pareto2.confint.rho.upper <- function(v, kappa, s, n, conf.level=0.95)
{
...
# 	if (v < 1e-13) return(0.0);
	...
	xsol_initial <- uniroot(function(x,v,kappa,s,n,gamma) {
		psstat(v,n,ppareto2,kappa,x,s)-gamma;
	}, c(0,1e25),v,kappa,s,n,gamma, tol=1e-9, maxiter=1000)$root;

	xsol <- .argmaxle(function(x,v,kappa,s,n,gamma) {
		psstat(v,n,ppareto2,kappa,x,s)-gamma;
	}, c(max(0,xsol_initial-1e-6),xsol_initial+1e-6),v,kappa,s,n,gamma, tol=1e-11, maxiter=10000)$root;

	rho.get(ppareto2, kappa, xsol, s); # return value
}
...
#' \code{\link{pareto2.confint.rho}}, \code{\link{rho.get}}
#' @export
pareto2.confint.rho.lower <- function(v, kappa, s, n, conf.level=0.95)
{
...
#' \code{\link{pareto2.confint.rho}}, \code{\link{rho.get}}
#' @export
pareto2.confint.rho <- function(v, kappa, s, n, conf.level=0.95)
{
	gamma <- 1-conf.level;
	return(c(
		pareto2.confint.rho.lower(v,kappa,s,n,1-gamma*0.5),
		pareto2.confint.rho.upper(v,kappa,s,n,1-gamma*0.5)
	));
}
